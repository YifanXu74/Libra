...
from libra.data.datasets.base_dataset import LibraConcatDataset, LibraChainDataset
    ...
    for split_name in datasets:
        if split_name != "train":
            ...
            if len(iterable_datasets) == 1:
                ...
                chained_datasets = LibraChainDataset(iterable_datasets)
            else:
                chained_datasets = None
            
            if len(map_datasets) == 1:
                concat_datasets = map_datasets[0]
            elif len(map_datasets) > 1:
                concat_datasets = LibraConcatDataset(map_datasets)
            else:
                concat_datasets = None

            if hasattr(chained_datasets, "__len__"):
                for ds in chained_datasets.datasets:
                    if not hasattr(ds, "__len__"):
                        delattr(chained_datasets.__class__, "__len__")
                        break
